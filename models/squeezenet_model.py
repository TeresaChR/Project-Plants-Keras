...
class SqueezeNet_model(BaseModel):

    def __init__(self, config):
        super(SqueezeNet_model, self).__init__(config)
        self.build_model()
    ...
    def SqueezeNet(self, include_top=True, weights='imagenet',
        ...
        x = self.fire_module(x, fire_id=2, squeeze=16, expand=64)
        x = self.fire_module(x, fire_id=3, squeeze=16, expand=64)
        x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), name='pool3')(x)
    
        x = self.fire_module(x, fire_id=4, squeeze=32, expand=128)
        x = self.fire_module(x, fire_id=5, squeeze=32, expand=128)
        x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), name='pool5')(x)
    
        x = self.fire_module(x, fire_id=6, squeeze=48, expand=192)
        x = self.fire_module(x, fire_id=7, squeeze=48, expand=192)
        x = self.fire_module(x, fire_id=8, squeeze=64, expand=256)
        x = self.fire_module(x, fire_id=9, squeeze=64, expand=256)
